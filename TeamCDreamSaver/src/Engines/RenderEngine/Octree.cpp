...
void Octree::Remove(IRenderable *r)
{
	...
	if(parent != current && parent != 0)
	{
		...
		if(parent->blf->hasChildren ||
			parent->blb->hasChildren ||
			parent->brb->hasChildren ||
			parent->brf->hasChildren ||
			parent->tlb->hasChildren ||
			parent->tlf->hasChildren ||
			parent->trb->hasChildren ||
			parent->trf->hasChildren)
		{
			noChildren = false;
		}

		if(total < maxPerNode && noChildren)
		{
