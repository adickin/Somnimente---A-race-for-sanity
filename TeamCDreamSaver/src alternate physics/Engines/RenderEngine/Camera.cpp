...
Camera::Camera(float fovy, float width, float height, float near, float far)
{
	...
	prevPos[0] = glm::vec3(0);
	prevPos[1] = glm::vec3(0);
	prevPos[2] = glm::vec3(0);
	currPosOffset = 0;
}
...
void Camera::Update(float elapsedMilliseconds)
{
	...
	prevPos[currPosOffset] = goalPosition;
	currPosOffset++;
	if(currPosOffset == 30)
		currPosOffset = 0;
	...
	glm::vec3 offset = a - position;
	//glm::vec3 offset = goalPosition - position;
/*
	glm::vec3 a(offset.x - prevOffset.x, offset.y - prevOffset.y, offset.z - prevOffset.z);
	
	if(glm::length(a) < 20.0f)
		offset = prevOffset;
